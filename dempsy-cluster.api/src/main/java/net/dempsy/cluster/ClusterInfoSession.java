...
public interface ClusterInfoSession {
     ...
     * 
     * @param path
     *            a '/' separated path to a directory in the cluster information manager.
     * 
     ...
     *             on an error which can include the fact that the parent directory doesn't exist.
     */
    ...
    /**
     * Check if a path has already been created. If the path exists, and if watcher is non-null then the watcher will be called back whenever the node at the path has data added to it, or is deleted.
     * 
     * @param path
     ...
     * 
     * @throws ClusterInfoException
     *             if there is an unforseem problem.
     */
    public boolean exists(String path, ClusterInfoWatcher watcher) throws ClusterInfoException;

    /**
     * If data is stored at the node indicated by the path, then the data will be returned. If the node e
     * 
     * @param path
     * @param watcher
