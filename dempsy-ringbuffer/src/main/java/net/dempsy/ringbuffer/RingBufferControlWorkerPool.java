 ...
 * 
 * <p>
 * Currently it would be really bad for Worker consumers to mix availableTo and tryAvailableTo. If a worker uses tryAvailableTo it MUST use
 * tryAvailableTo until tryAvailableTo returns a value that isn't RingBufferConsumerControl.UNAVAILABLE before using availableTo.
 * </p>
 */
    ...
    private RingBufferConsumerControl createConsumerControl() {
        return new RingBufferConsumerControl(sizePowerOfTwo, waitStrategy, cursor, commonStop) {
            ...
            public long tryAvailableTo() {
                ...
                if (alt == RingBufferConsumerControl.UNAVAILABLE) return alt;
...
//            next = next();
        ...
        publish(Long.MAX_VALUE - 2); /// jump to the end but don't interfere with the initial value which is set to Long.MAX_VALUE
        return next;
    }
